...
class EventService {
  ...
  // Obtener eventos sugeridos por un usuario
  List<Event> getUserSuggestedEvents(String userId) {
    return _events.where((e) => e.suggestedByIds.contains(userId)).toList();
  }
  ...
  // Sugerir evento
  Event? suggestEvent(String eventId, String userId) {
    final index = _events.indexWhere((e) => e.id == eventId);
    if (index == -1) return null;

    if (_events[index].suggestedByIds.contains(userId)) return _events[index];

    final suggested = [..._events[index].suggestedByIds, userId];
    final updatedEvent = _events[index].copyWith(
      suggestedByIds: suggested,
      updatedAt: DateTime.now(),
    );
  ...
  // Remover sugerencia de evento
  Event? removeSuggestEvent(String eventId, String userId) {
    final index = _events.indexWhere((e) => e.id == eventId);
    if (index == -1) return null;

    final suggested = _events[index].suggestedByIds
        .where((id) => id != userId)
        .toList();
...
void initWithMockData() {
  ...
  _events.addAll([
    Event(
      ...
      description: 'Partido amistoso para todos los estudiantes interesados en fútbol. Se juega en el patio principal.',
    ...
    Event(
      ...
      description: 'Competencia de voleibol entre los diferentes cursos. Inscripción por equipos de 6 personas.',
    ...
    Event(
      ...
      description: 'Sesión de debate sobre temas de actualidad. Mejora tu oratoria y argumentación.',
    ...
    Event(
      ...
      description: 'Carrera de velocidad abierta para todos los estudiantes. Hay premios para los ganadores.',
    ...
    Event(
      ...
      description: 'Presenta tus proyectos científicos y aprende de otros estudiantes. Muestra de trabajos prácticos.',
